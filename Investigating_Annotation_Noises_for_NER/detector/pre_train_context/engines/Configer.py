...
class Configer:
    def __init__(self, config_file='system.config'):
        ...
        the_item = 'max_char_len'
        if the_item in config:
            self.max_char_len = int(config[the_item])

        the_item = 'char_dim'
        if the_item in config:
            self.char_dim = int(config[the_item])

        the_item = 'char_lstm_dim'
        if the_item in config:
            self.char_lstm_dim = int(config[the_item])
    ...
    def show_data_summary(self, logger):
        ...
        logger.info("     max nodes length: %s" % (self.max_sequence_length))
