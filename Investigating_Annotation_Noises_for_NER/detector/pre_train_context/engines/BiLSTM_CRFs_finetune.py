...
class BiLSTM_CRFs(object):
    ...
    # def get_distance_of_mention(self):
    #     forward_p = [1 - 0.1 * ((10 - i) - 1) for i in range(10)]
    #     backward_p = sorted(forward_p, reverse=True)
    #     forward_p = self.batch_size*[forward_p]
    #     backward_p = self.batch_size*[backward_p]
    #     forward_p = tf.convert_to_tensor(np.array(forward_p).astype(np.float32))
    #     backward_p = tf.convert_to_tensor(np.array(backward_p).astype(np.float32))
    #     forward_p = tf.expand_dims(forward_p,-1)
    #     backward_p = tf.expand_dims(backward_p,-1)
    #     return forward_p,backward_p
    #
    # def liner_layer(self):
    #     forward_p, backward_p = self.get_distance_of_mention()
    #     with tf.variable_scope("liner"):
    #         liner_w = tf.get_variable("w", [1, self.attention_dim],
    #                                     initializer=self.initializer)
    #         liner_b = tf.get_variable("b", [self.attention_dim], initializer=self.initializer)
    #         forward_p_input = tf.matmul(forward_p,liner_w) + liner_b
    #         backward_p_input = tf.matmul(backward_p,liner_w) + liner_b
    #         return forward_p_input,backward_p_input
    ...
    def build(self):
        ...
        self.optimizer_g = tf.train.AdamOptimizer(
            ...
            self.combain_loss + self.c_loss - self.m_loss,
            var_list=var_g,
            global_step=self.global_step
        )  # optimized generator
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            self.logger.info("time consumption:%.2f(min), validation combain loss: %.5f, validation combain macro-f1: %.5f, validation context loss: %.5f, validation context macro-f1: %.5f, validation mention loss: %.5f, validation mention macro-f1: %.5f" %
                             (time_span, np.array(dev_combain_losses).mean(), dev_combain_results['f1'], np.array(dev_context_losses).mean(), dev_context_results['f1'], np.array(dev_mention_losses).mean(), dev_mention_results['f1']))

            dev_acc_avg = dev_context_results['f1']
            gap_f1 = dev_context_results['f1']-dev_mention_results['f1']
            if dev_acc_avg > self.best_acc_dev or gap_f1 > self.biggest_gap:
                unprogressed = 0
                self.best_acc_dev = dev_acc_avg
                self.biggest_gap = gap_f1
                best_at_epoch = epoch
                saver.save(self.sess, self.checkpoints_dir + "/" + self.checkpoint_name, global_step=self.global_step)
                self.logger.info("saved the new best model with combain macro-f1 is: %.3f, with context macro-f1 is: %.3f, with mention macro-f1 is: %.3f, the differ of context and mention is: %.3f" % (self.best_acc_dev,dev_context_results['f1'],dev_mention_results['f1'],self.biggest_gap))
            else:
                unprogressed += 1
    ...
    def test(self):
        ...
        for i in range(num_iterations):
            ...
            test_y_pred,test_c_pred,test_c_score = \
                self.sess.run([
                    self.y_pred,self.c_pred,self.c_scores
                ],
                    feed_dict={
            ...
            for score_list in test_c_score:
                test_c_O_score.append(score_list[0])
                test_c_M_score.append(score_list[1])
                test_c_D_score.append(score_list[2])
                test_c_C_score.append(score_list[3])
        ...
        self.logger.info("time consumption:%.2f(min), test combain macro-f1: %.5f; test context %s" %
        ...
        def output_test_results(sens_ids,entities_ids, sentences, entities, types_pred, types_real, M_score, D_score, C_score,
                                O_score, output_filename, is_context=False):
