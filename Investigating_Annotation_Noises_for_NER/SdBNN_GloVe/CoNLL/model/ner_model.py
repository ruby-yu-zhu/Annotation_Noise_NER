...
class NERModel(BaseModel):
    ...
    def data_uncertainty_loss(self, T):
        ...
        self.loss =  uncertainly_loss + softmax_loss
    ...
    def run_test_evaluate(self, test):
        """Evaluates performance on test set
        ...
        """
        accs = []
        PER_correct_preds, PER_total_correct, PER_total_preds = 0., 0., 0.
        ORG_correct_preds, ORG_total_correct, ORG_total_preds = 0., 0., 0.
        LOC_correct_preds, LOC_total_correct, LOC_total_preds = 0., 0., 0.
        MISC_correct_preds, MISC_total_correct, MISC_total_preds = 0., 0., 0.
        entities_correct_preds, entities_total_correct, entities_total_preds = 0., 0., 0.
        # all_labs,all_pred_labels = [],[]
        ...
        for words, labels,softmax_pc,softmax_pm in minibatches(test, self.config.batch_size):
            ...
            for lab, lab_pred, length in zip(labels, labels_pred,
                ...
                accs    += [a==b for (a, b) in zip(lab, lab_pred)]
        #         all_labs.append(lab)
        #         all_pred_labels.append(lab_pred)
        # all_pred_labels = all_pred_labels[0:len(test)]
        # all_labs = all_labs[0:len(test)]
        # judge_metrics = metrics(all_labs,all_pred_labels,self.config.vocab_tags)
        # return judge_metrics


                lab_chunks      = get_chunks(lab, self.config.vocab_tags)
                lab_pred_chunks = get_chunks(lab_pred,
                                                 self.config.vocab_tags)
                PER_lab_chunks,PER_lab_pred_chunks,LOC_lab_chunks,LOC_lab_pred_chunks,ORG_lab_chunks,ORG_lab_pred_chunks,\
                MISC_lab_chunks,MISC_lab_pred_chunks = [],[],[],[],[],[],[],[]
                for lab_chunk in lab_chunks:
                    if lab_chunk[0] == 'PER':
                        PER_lab_chunks.append(lab_chunk)
                    elif lab_chunk[0] == 'ORG':
                        ORG_lab_chunks.append(lab_chunk)
                    elif lab_chunk[0] == 'LOC':
                        LOC_lab_chunks.append(lab_chunk)
                    elif lab_chunk[0] == 'MISC':
                        MISC_lab_chunks.append(lab_chunk)
                for lab_pred_chunk in lab_pred_chunks:
                    if lab_pred_chunk[0] == 'PER':
                        PER_lab_pred_chunks.append(lab_pred_chunk)
                    elif lab_pred_chunk[0] == 'ORG':
                        ORG_lab_pred_chunks.append(lab_pred_chunk)
                    elif lab_pred_chunk[0] == 'LOC':
                        LOC_lab_pred_chunks.append(lab_pred_chunk)
                    elif lab_pred_chunk[0] == 'MISC':
                        MISC_lab_pred_chunks.append(lab_pred_chunk)

                PER_correct_preds += len(set(PER_lab_chunks) & set(PER_lab_pred_chunks))
                PER_total_preds   += len(PER_lab_pred_chunks)
                PER_total_correct += len(PER_lab_chunks)

                ORG_correct_preds += len(set(ORG_lab_chunks) & set(ORG_lab_pred_chunks))
                ORG_total_preds += len(ORG_lab_pred_chunks)
                ORG_total_correct += len(ORG_lab_chunks)

                LOC_correct_preds += len(set(LOC_lab_chunks) & set(LOC_lab_pred_chunks))
                LOC_total_preds += len(LOC_lab_pred_chunks)
                LOC_total_correct += len(LOC_lab_chunks)

                MISC_correct_preds += len(set(MISC_lab_chunks) & set(MISC_lab_pred_chunks))
                MISC_total_preds += len(MISC_lab_pred_chunks)
                MISC_total_correct += len(MISC_lab_chunks)

                entities_correct_preds += PER_correct_preds+ORG_correct_preds+LOC_correct_preds+MISC_correct_preds
                entities_total_preds += PER_total_preds+ORG_total_preds+LOC_total_preds+MISC_total_preds
                entities_total_correct += PER_total_correct+ORG_total_correct+LOC_total_correct+MISC_total_correct

        PER_p   = PER_correct_preds / PER_total_preds if PER_correct_preds > 0 else 0
        PER_r   = PER_correct_preds / PER_total_correct if PER_correct_preds > 0 else 0
        PER_f1  = 2 * PER_p * PER_r / (PER_p + PER_r) if PER_correct_preds > 0 else 0

        ORG_p = ORG_correct_preds / ORG_total_preds if ORG_correct_preds > 0 else 0
        ORG_r = ORG_correct_preds / ORG_total_correct if ORG_correct_preds > 0 else 0
        ORG_f1 = 2 * ORG_p * ORG_r / (ORG_p + ORG_r) if ORG_correct_preds > 0 else 0

        LOC_p = LOC_correct_preds / LOC_total_preds if LOC_correct_preds > 0 else 0
        LOC_r = LOC_correct_preds / LOC_total_correct if LOC_correct_preds > 0 else 0
        LOC_f1 = 2 * LOC_p * LOC_r / (LOC_p + LOC_r) if LOC_correct_preds > 0 else 0

        MISC_p = MISC_correct_preds / MISC_total_preds if MISC_correct_preds > 0 else 0
        MISC_r = MISC_correct_preds / MISC_total_correct if MISC_correct_preds > 0 else 0
        MISC_f1 = 2 * MISC_p * MISC_r / (MISC_p + MISC_r) if MISC_correct_preds > 0 else 0

        p = entities_correct_preds / entities_total_preds if entities_correct_preds > 0 else 0
        r = entities_correct_preds / entities_total_correct if entities_correct_preds > 0 else 0
        f1 = 2 * p * r / (p + r) if entities_correct_preds > 0 else 0
        acc = np.mean(accs)

        return { "f1": 100*f1, "percision":100*p, "recall":100*r,"PER_f1": 100*PER_f1, "PER_percision":100*PER_p, "PER_recall":100*PER_r,
                 "ORG_f1": 100*ORG_f1, "ORG_percision":100*ORG_p, "ORG_recall":100*ORG_r,"LOC_f1": 100*LOC_f1, "LOC_percision":100*LOC_p, "LOC_recall":100*LOC_r,
                 "MISC_f1": 100*MISC_f1, "MISC_percision":100*MISC_p, "MISC_recall":100*MISC_r}
