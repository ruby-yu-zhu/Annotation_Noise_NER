...
class SequenceTagger(flair.nn.Model):
    def __init__(
        ...
        if self.use_rnn:
            ...
            self.system_error_Liner1 = torch.nn.Linear(len(tag_dictionary), len(tag_dictionary),bias=False)
            self.system_error_Liner2 = torch.nn.Linear(len(tag_dictionary), len(tag_dictionary),bias=False)
        else:
            self.linear = torch.nn.Linear(
        ...
        # self.var_Liner = torch.nn.Linear(512, 1, bias=True)
    ...
    def forward_loss(
        ...
        features,system_error = self.forward(data_points)
        # print("the features shape is: ",features.shape)

        return self._calculate_train_loss(features,system_error, data_points,l=0.5)
    ...
    def forward(self, sentences: List[Sentence]):
        ...
        all_diff_c = []
        all_diff_m = []
        ml = max([len(sentence) for sentence in sentences])
        # print("the max_seq_len in the batch is: ", ml)
        for sentence in sentences:
            # get softmax_clear_list-----------
            sentence_diff_c = []
            sentences_diff_m = []
            for token in sentence:  
                diff_c_dict = eval(token.get_tag('diff_c').value) #每个句子中每个类别（LOC, PER, ORG, MISC）的上下文相关系统偏差
                diff_m_dict = eval(token.get_tag('diff_m').value)
                diff_c_list = 20 * [0.0]
                diff_m_list = 20 * [0.0]
                for key, value in diff_c_dict.items():
                    value = float(value)
                    diff_c_list[self.tag_dictionary.get_idx_for_item('B-' + key)] = value
                    diff_c_list[self.tag_dictionary.get_idx_for_item('I-' + key)] = value
                    diff_c_list[self.tag_dictionary.get_idx_for_item('E-' + key)] = value
                    diff_c_list[self.tag_dictionary.get_idx_for_item('S-' + key)] = value
                for key, value in diff_m_dict.items():
                    value = float(value)
                    diff_m_list[self.tag_dictionary.get_idx_for_item('B-' + key)] = value
                    diff_m_list[self.tag_dictionary.get_idx_for_item('I-' + key)] = value
                    diff_m_list[self.tag_dictionary.get_idx_for_item('E-' + key)] = value
                    diff_m_list[self.tag_dictionary.get_idx_for_item('S-' + key)] = value
                sentence_diff_c.append(diff_c_list)   # diff_c_list不是列表格式的,是列表元素,浮点型, sentence_diff_c是二维列表20*sentence
                sentences_diff_m.append(diff_m_list)  
                # print(token,token.get_tag(self.tag_type).value,diff_c_dict,diff_c_list)
            for i in range(ml - len(sentence)):
                padding_list = 20 * [0.0]
                sentence_diff_c.append(padding_list)  
                sentences_diff_m.append(padding_list)
            all_diff_c.append(sentence_diff_c) # 20*ml
            all_diff_m.append(sentences_diff_m)
            all_embs += [
        ...
        if self.use_rnn:
            ...
            if self.use_locked_dropout > 0.0:
                sentence_tensor = self.locked_dropout(sentence_tensor)
        # print("the output from bilstm is: ",sentence_tensor.shape)
        all_diff_c = torch.tensor(all_diff_c, device=flair.device)
        all_diff_m = torch.tensor(all_diff_m, device=flair.device)

        features = self.linear(sentence_tensor)
        # print('the shape of features is:',features.shape)
        # vars_pred = self.var_Liner(sentence_tensor)
        # vars = self.var_Softplus(vars_pred)
        # features_vars = torch.cat([features, vars], -1)
        system_error = self.system_error_Liner1(all_diff_c) + self.system_error_Liner2(all_diff_m)  #batch_size*ml*20
        features = features+system_error

        # print("the var and system_error shape: ", features.shape,vars.shape, system_error.shape)
        return features,system_error
    ...
    def _calculate_train_loss(self,feature,system_error,data_points,l):
        loss1 = self._calculate_loss(feature,data_points)
        reg_l2 = torch.norm(system_error,dim=-1)  #默认求指定维度二范数
        reg_l2 = torch.sum(reg_l2,dim=-1) 
        reg_l2 = torch.mean(reg_l2)
        return loss1+l*reg_l2
